...
// normalizes the file name to remove unprintable and extra characters
function normalizeFileName(fname) {
  return ru(ra(fname.normalize('NFC'))).trim();
}
...
async function uploadAttachmentsToNextCloud(folder, attachments) {
  ...
  for (const attachment of attachments) {
    const fname = normalizeFileName(attachment.parameters.name);
...
function getTempFileName(fname) {
  return normalizeFileName(
    path.join(os.tmpdir(), `${Date.now()}-${fname}`),
  );
}
