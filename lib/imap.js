...
async function processQueueOfMessages(server, queue) {
  ...
  db.prepare('UPDATE smtp SET active = TRUE WHERE id = ?').run(server.id);
  ...
  const promises = queue
    .map(async ({ message, attachments }) => {
      ...
      // check to make sure that the mesasage isn't a comment from freshdesk
      const sender = message.envelope.from.pop();
      const receiver = message.envelope.to.pop();

      if (sender.address.includes('freshdesk')) {
        debug('Message is from freshdesk. Skipping...');
        db.prepare('UPDATE inbox SET seq = (SELECT MAX(seq, ?) FROM inbox WHERE smtp_id = ?) WHERE smtp_id = ?;')
          .run(message.seq, server.id, server.id);
        return;
      }

      // make sure the email has an attachment.
      if (attachments.length === 0) {
        debug('Message does not have an attachment.  Skipping...');
        db.prepare('UPDATE inbox SET seq = (SELECT MAX(seq, ?) FROM inbox WHERE smtp_id = ?) WHERE smtp_id = ?;')
          .run(message.seq, server.id, server.id);
        return;
      }
  ...
  try {
    ...
    db.prepare('UPDATE smtp SET active = FALSE WHERE id = ?').run(server.id);
  }
}
...
async function main() {
  const servers = db.prepare('SELECT * FROM smtp WHERE paused = FALSE AND active = FALSE;').all();
