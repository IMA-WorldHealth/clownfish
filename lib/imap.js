...
async function processQueueOfMessages(server, queue) {
  ...
  const promises = queue
    .map(async ({ message, attachments }) => {
      ...
      debug(`Using parent folder: ${server.folder}`);

      // ensures that this folder exists and returns its ID.
      const gFolderId = await api.ensureFolder(normalizedStructure, server.folder);

      debug(`Located folder for ${normalizedStructure} with id ${gFolderId}. Uploading attachments...`);
      await utils.uploadAttachmentsToGoogleDrive(attachments, normalizedReportName, gFolderId);
      ...
      const params = [
        ...
        message.envelope.subject, fnames, Number(date), gFolderId,
      ];
